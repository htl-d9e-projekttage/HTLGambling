...
export class Slots {
	...
	symbols = {
		lo: {
			...
			html: { chance: 0.2, multiplier: 5 },
			go: { chance: 0.15, multiplier: 7 },
			github: { chance: 0.12, multiplier: 10 },
			intellij: { chance: 0.1, multiplier: 15 },
			vsc: { chance: 0.1, multiplier: 20 },
			c: { chance: 0.08, multiplier: 25 } // Least frequent in `lo`
		},

		mid: {
			swift: { chance: 0.3, multiplier: 40 }, // Most frequent in `mid`
			mongodb: { chance: 0.25, multiplier: 50 },
			css: { chance: 0.2, multiplier: 60 },
			python: { chance: 0.15, multiplier: 80 },
			discord: { chance: 0.1, multiplier: 100 } // Least frequent in `mid`
		},

		hi: {
			arch: { chance: 0.4, multiplier: 500 }, // Most frequent in `hi`
	...
	private getRandomSymbolFromGrade(grade: {
		[key: string]: { chance: number; multiplier: number };
	}): string {
	...
	private generateRandomSymbol(): string {
		const random = this.genNum();
		let grade =
			random < 0.75 ? this.symbols.lo : random < 0.95 ? this.symbols.mid : this.symbols.hi;
